// src/llm/qwen_llm.cpp -- Alibaba Cloud Qwen (DashScope) API backend.
...
QwenLLM::QwenLLM(QwenLLMConfig cfg)
    : cfg_(std::move(cfg))
{
}

json QwenLLM::build_payload(const ChatRequest& req, bool stream) const
{
    return build_openai_payload(req, cfg_.model, stream, "/v1/chat/completions");
}

ChatResponse QwenLLM::invoke_impl(const ChatRequest& req)
{
    ...
    if (!res)
    {
        throw LCError("Qwen: request failed: " + httplib::to_string(res.error()));
    }
    if (res->status / 100 != 2)
    {
        throw LCError("Qwen: HTTP " + std::to_string(res->status) + ": " + res->body);
    }

    json j = json::parse(res->body, nullptr, false);
    if (j.is_discarded())
    {
        throw LCError("Qwen: invalid JSON response");
    }
...
ChatResponse QwenLLM::invoke_stream_impl(const ChatRequest& req,
                                        const StreamCallback& on_delta)
{
    ...
    if (!res)
    {
        throw LCError("Qwen stream: request failed: " + httplib::to_string(res.error()));
    }
    if (res->status / 100 != 2)
    {
        throw LCError("Qwen stream: HTTP " + std::to_string(res->status));
    }
