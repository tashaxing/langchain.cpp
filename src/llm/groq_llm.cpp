// src/llm/groq_llm.cpp -- Groq API backend (OpenAI-compatible, ultra-fast inference).
// https://groq.com/
...
GroqLLM::GroqLLM(GroqLLMConfig cfg)
    : cfg_(std::move(cfg))
{
}

json GroqLLM::build_payload(const ChatRequest& req, bool stream) const
{
    return build_openai_payload(req, cfg_.model, stream, "/v1/chat/completions");
}

ChatResponse GroqLLM::invoke_impl(const ChatRequest& req)
{
    ...
    if (!res)
    {
        throw LCError("Groq: request failed: " + httplib::to_string(res.error()));
    }
    if (res->status / 100 != 2)
    {
        throw LCError("Groq: HTTP " + std::to_string(res->status) + ": " + res->body);
    }

    json j = json::parse(res->body, nullptr, false);
    if (j.is_discarded())
    {
        throw LCError("Groq: invalid JSON response");
    }
...
ChatResponse GroqLLM::invoke_stream_impl(const ChatRequest& req,
                                        const StreamCallback& on_delta)
{
    ...
    if (!res)
    {
        throw LCError("Groq stream: request failed: " + httplib::to_string(res.error()));
    }
    if (res->status / 100 != 2)
    {
        throw LCError("Groq stream: HTTP " + std::to_string(res->status));
    }
